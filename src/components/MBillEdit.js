...
import Table, { SelectColumnFilter } from '../utils/GSTINTable'
		...
		if (id !== 'new') {
			...
			async function fetchPunchingMedium() {
				await axios.get('/pm/' + id)
					.then((response) => {
						console.log("punching medium data is" + response.data);
						...
						<Tab eventKey="page1" title="Bill Detail" className="h-120">
							...
							<div className="grid grid-cols-2 gap-0">
								...
								<div>
									...
									<input type="text" disabled name="unitFinCode" {...register("dak.unit.uuid")}
										className="form-control py-0"
									/>
									<div className="text-red-500">{errors.dak?.unit?.uuid?.message}</div>
								</div>
								...
								<div>
									<label>Bill No</label>
									<input type="text" name="billNo" {...register("dak.billNo",{required:"Bill No required"})}
										className="form-control py-0"
									/>
									<div className="text-red-500">{errors.dak?.billNo?.message}</div>
								</div>
								...
								<div>
									...
									<input type="date" name="billDate" {...register("dak.billDate")}
										className="form-control py-0"
									/>
									<div className="text-red-500">{errors.dak?.billDate?.message}</div>
								</div>
								<div>
							...
							<div >
								<Table vendorGstinList={vendorGstinDetails} className="table-auto" />
							</div>
