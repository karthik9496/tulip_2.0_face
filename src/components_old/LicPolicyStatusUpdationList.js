...
function Lic() {
	...
	useEffect(() => {
		...
		async function fetchData() {
			if(!fetching)
			await axios.get('/lics/policyStatusUpdation/lic?cdaoNo='+cdaoNo+'&policyNo='+policyNo+'&policyAmount='+policyAmount+'&premiumAmount='+premiumAmount)
				.then((response) => {
	...
	const columns = useMemo(() => [
			...
			Cell: ({ row }) => (
					...
					<Link to={"/lics/policyStatusUpdation/audSubmit/" + row.original.id}>
		...
		<div className="min-h-screen bg-gray-100 text-gray-900">
			<main className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
				<div className="mt-2 ml-4">
					<h1 className="text-xl font-semibold">Lic Policy Status Updation</h1>
					...
					{usrLevel!==null && usrLevel<30 &&
					<h1  className="text-xl font-semibold">Search by Policy No or CdaoNo or Both </h1>	
					}
