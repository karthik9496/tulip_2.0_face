...
const RbsTransEdit = () => {
    ...
    const [bdVoucher,setBdVoucher]=useState(false);
    const [drr,setDrr]=useState(false);
    
    const [drrR,setDrrR]=useState('');
    const [drrS,setDrrS]=useState('');
    const [drrApp,setDrrApp]=useState('');
			...
			async function fetchData() {
				...
				await axios.get('/rbsTrans/' + id)
					.then((response) => {
						...
						if(record.rentBillSingleRevision==='BDV'){
							setBdVoucher(true);
						}
						
						if(record.rentBillSingleRevision==='DRR'){
							setDrr(true);
						}
						const fields = [
							...
							'rejectionReason','uploadFileName','rentBillNo','rentBillDate','storesBrief','quantity','rate','bsoBdAmount',
							'approvedBdAmount','drrFromDate','drrToDate','drrRate','drrRent','drrServantGarage','drrAmount','drrBsoAmount'
	...
	const onSubmit = (data, event) => {
		...
		console.log(data);
		if (data.id && bdVoucher===false && drr===false) {
			axios.put("/rbsTrans/" + data.id, data)
				.then((response) => {
		...
		}else if (data.id && bdVoucher===true) {
			console.log(bdVoucher);
			axios.put("/rbsTrans/bdVoucher/aud/" + data.id, data)
				.then((response) => {
					setMesg(response.data);
		...
		}else if (data.id && drr===true) {
			console.log(drr);
			axios.put("/rbsTrans/drr/aud/" + data.id, data)
				.then((response) => {
					console.log("----------values retd----log------:" + response.data);
					setMesg(response.data);
			//		console.log("----------values retd-------:" + response.data['drrRent']);
			//		setDrrR(response.data['drrRent']);
			//		setDrrS(response.data['drrServantGarage']);
			//		setDrrApp(response.data['drrAmount']);
	...
	const handleButtonClick = (e) =>{
		...
		<div className="min-h-screen bg-gray-100 text-gray-900">
			<div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
				...
				{bdVoucher===true &&
				<>
					<h1 >{id === 'bdv' ? 'Add' : 'Edit'} Barrack Damage Voucher </h1>
					</>
				}
				{drr===true &&
				<>
					<h1 >{id === 'drr' ? 'Add' : 'Edit'} Damage Rate of Rent </h1>
					</>
				}
						...
						<Tab eventKey="page1" title="IOR" className="h-120">
							<div className="grid grid-cols-3 gap-0">
								...
								<div>
									...
									<input type="date" name="excessFurToDate" {...register("excessFurToDate")}
										className="form-control py-0" />
									<div className="text-red-500">{errors.excessFurToDate?.message}</div>
								</div>
								...
								<>
								<div>
									<label>Stores Brief </label>
									<input type="text" name="storesBrief" {...register("storesBrief")}
										className="form-control py-0"
									/>
									<div className="text-red-500">{errors.storesBrief?.message}</div>
								</div>
								...
								<div>
									<label>Rate</label>
									<input type="text" name="rate" {...register("rate")}
										className="form-control py-0"
									/>
									<div className="text-red-500">{errors.rate?.message}</div>
								</div>
								<div>
									<label>BSO BD Amount </label>
									<input type="text" name="bsoBdAmount" {...register("bsoBdAmount")}
										className="form-control py-0"
									/>
									<div className="text-red-500">{errors.bsoBdAmount?.message}</div>
								</div>
								
								<div>
									<label>Approved BD Amount </label>
									<input type="text" name="approvedBdAmount" {...register("approvedBdAmount")}
										className="form-control py-0"
									/>
									<div className="text-red-500">{errors.approvedBdAmount?.message}</div>
								</div>
								...
								<div className="grid grid-cols-3 gap-0">
								<div>
									<label>Drr Bso Amount</label>
									<input type="text" name="drrBsoAmount" {...register("drrBsoAmount")}
										className="form-control py-0"
									/>
									<div className="text-red-500">{errors.drrBsoAmount?.message}</div>
								</div>
								<div>
								...
								<div>
									<label>Drr From Date</label>
									<input type="date" name="drrFromDate" {...register("drrFromDate")}
										className="form-control py-0" />
									<div className="text-red-500">{errors.drrFromDate?.message}</div>
								</div>
								
								<div>
									<label>Drr To Date</label>
									<input type="date" name="drrToDate" {...register("drrToDate")}
										className="form-control py-0" />
									<div className="text-red-500">{errors.drrToDate?.message}</div>
								</div>
								
								<div>
									<label>Drr Lf Rate</label>
									<input type="text" name="lfeeAmount" {...register("lfeeAmount")}
										className="form-control py-0"
									/>
									<div className="text-red-500">{errors.lfeeAmount?.message}</div>
								...
								<div>
									<label>Drr Amount </label>
									<input type="text" name="drrAmount" {...register("drrAmount")}
										className="form-control py-0"
									/>
									<div className="text-red-500">{errors.drrAmount?.message}</div>
								</div>
...
export default withRouter(RbsTransEdit);