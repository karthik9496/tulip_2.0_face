...
function DidsList() {
 ...
 async function rollBack(id) {
    await axios.put(`/dids/rollback/${id}`)
      .then((response) => {
        //console.log(data);
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      if (true) {
        await axios
          .get("/dids/pending?search=" + search)
          .then((response) => {
              ...
              row.original.aaoDate === null && (
                <div>
                  ...
                  <Link to={"/dids/" + row.original.id}>
                   ...
                   <button className="w-16 m-0 p-0 bg-green-500 hover:bg-green-700 "  onClick={() => rollBack(row.original.id)}>
                      {" "}
                      Rollback{" "}
                    </button>
                </div>
...
export default withRouter(DidsList);
