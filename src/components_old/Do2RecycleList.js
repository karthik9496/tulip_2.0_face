...
function Do2RecycleList() {
	...
	useEffect(() => {
		...
		async function fetchData() {
			setLoading(true);
			if(!fetching)
			await axios.get('/do2s/recycle/pending?cdaoNo='+search+'&desc='+desc+'&icno='+icno)
				.then((response) => {
					if(response.data[0])
	...
	const columns = useMemo(() => [
		{
			...
			Cell: ({ row }) => (
					...
					<Link to={{pathname:"/do2s/"+row.original.id, secState:'lw-recycle'}}>
		...
		<div className="min-h-screen bg-gray-100 text-gray-900">
			<main className="max-w-11/12xl mx-auto px-4 sm:px-6 lg:px-8 pt-0">
				<div className="mt-2 ml-4">
					<h1 className="text-xl font-semibold">Do2s Recycle List</h1>
...
export default withRouter(Do2RecycleList);
