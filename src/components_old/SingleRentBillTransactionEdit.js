...
const SingleRentBillTransactionEdit = () => {
	...
	useEffect(() => {
		...
		if (id !== 'new') {
			async function fetchData() {
				let record = '';
				await axios.get('/singleRentBillTransactions/' + id)
					.then((response) => {
	...
	const onSubmit = (data, event) => {
		...
		console.log(data);
		if (data.id) {
			axios.put("/singleRentBillTransactions/" + data.id, data)
				.then((response) => {
			...
			axios.post("/singleRentBillTransactions", data)
				.then((response) => { 
	...
	const handleButtonClick = (e) =>{
		history.push("/singleRentBillTransactions");
	}
...
export default withRouter(SingleRentBillTransactionEdit);