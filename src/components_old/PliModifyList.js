...
function PliModifyList() {
	...
	useEffect(() => {
		...
		async function fetchData() {
			if(!fetching)
			await axios.get('/plis/modifyList?cdaoNo='+search+'&policyNo='+policyNo)
				.then((response) => {
		 ...
		 
		 {
			...
			Cell: ({ row }) => (
				<div>
					...
					<Link to={"/plis/modify/" + row.original.id}>
						<button className=" w-16 m-0 p-0 " > Modify </button>
					</Link>
		...
		<div className="min-h-screen bg-gray-100 text-gray-900">
			<main className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
				<div className="mt-2 ml-4">
					<h1 className="text-xl font-semibold">Plis - Modify List</h1>
...
export default withRouter(PliModifyList);
