...
const RefundTransEdit = () => {
	...
	const onSubmit = (data, event) => {
		...
		setDisabled(true); 
 		console.log(data);
		if (data.id) {
			axios.put("/refundTrans/" + data.id, data)
				.then((response) => {
	...
	const handleButtonClick = (e) =>{
		history.push("/refundTransactions");
	}
		...
		<div className="min-h-screen bg-gray-100 text-gray-900">
			<div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
				...
				<form onSubmit={handleSubmit(onSubmit, onError)}>
					<h1 >{id === 'new' ? 'Add' : 'Edit'} Refund Transaction Edit </h1>
...
export default withRouter(RefundTransEdit);