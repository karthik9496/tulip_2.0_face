...
const LetterDprSectionWise = () => {
    ...
    async function fetchData() {
      ...
      try {
        const response = await axios.get(
          `/letters/dpr/sectionWise?fromDate=${fromDateStr}&toDate=${toDateStr}`
        );
  ...
  async function generateSectionWiseDprPdf(fromDateStr, toDateStr) {
    setLoading(true);
    setGeneratingPdf(true);

    try {
      const response = await axios.get(
        `/letters/print/sectionWiseDpr?fromDate=${fromDateStr}&toDate=${toDateStr}`
      );
      window.open(
  ...
  const ShowLetterDPR = () => {
    ...
    async function breakUp(section, fromDateStr, toDateStr) {
      try {
        const response = await axios.get(
          `/letters/dpr/sectionWise/${section}/dprBreakup?fromDateStr=${fromDateStr}&toDateStr=${toDateStr}`
        );
        if (response.data[0]) {
    ...
    return (
      <div className="min-h-full bg-gray-100 text-gray-900">
        ...
        <main id="dprBreakUp" className="pt-8">
          <div className="min-h-full bg-gray-100 text-gray-900">
    ...
    <div className="min-h-screen max-h-full bg-gray-100 text-gray-900">
      ...
      >
        <h1 className="text-3xl font-semibold mx-auto">
          Letter DPR - Section Wise
        </h1>
        {serverErrors && (
          <p className="font-semibold text-red-500 mx-auto text-xl">
            {serverErrors}
          </p>
        )}
...
export default withRouter(LetterDprSectionWise);
