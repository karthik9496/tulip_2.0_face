...
const RefundTransactionEdit = () => {
	...
	useEffect(() => {
		...
		if (id !== 'new') {
			async function fetchData() {
				let record = '';
				await axios.get('/refundTransactions/' + id)
					.then((response) => {
	...
	const onSubmit = (data, event) => {
		...
		console.log(data);
		if (data.id) {
			axios.put("/refundTransactions/" + data.id, data)
				.then((response) => {
...
export default withRouter(RefundTransactionEdit);