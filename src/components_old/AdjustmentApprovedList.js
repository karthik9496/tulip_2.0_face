...
function AdjustmentApprovedList() {
	...
	useEffect(() => {
		...
		async function fetchData() {
			console.log(qe);
			if(!fetching)
			await axios.get('/adjustmentCbs/section/approved?search='+search+'&qe='+qe)
				.then((response) => {
	...
	const columns = useMemo(() => [				
		{
			...
			Cell: ({ row }) => (
				  ...
				  <div>
				   {row.original.armyNo}{row.original.checkDigit}<br/>
		...
		<div className="min-h-screen bg-gray-100 text-gray-900">
			<main className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
				<div className="mt-2 ml-4">
					<h1 className="text-xl font-semibold">Adjustment Cbs - Approved</h1>
					...
					<div>
						...
						onChange={e => setInputText(e.target.value)}
							...
							<div>
							<input type="text" name="qe" placeholder="qe mm/yyyy" onChange={e => setQe(e.target.value)} />
							</div>
						<button type="submit" onClick={handleSubmit} className="w-16 m-0 p-0">Search</button>
						<div>
							<Link to={"/adjustmentCbs"}>
...
export default withRouter(AdjustmentApprovedList);
