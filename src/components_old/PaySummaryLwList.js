...
function PaySummaryLwList() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      if (!fetching)
        await axios
          .get("/paySummarys/lw?cdaoNo=" + cdaoNo+'&me='+me+'&rankCode='+rankCodeSearch)
          .then((response) => {
   ...
   async function mps(id) {
		...
		setData(newData);
		 	...
		 	setMpsDisabled(true);
		 await axios.put(`/paySummarys/mps/draft/${id}`)
			.then((response) => {
  ...
  async function reclose(id) {
		...
		setData(newData);
		 	...
		 	setDisabled(true);
		 await axios.put(`/paySummarys/accountsClosing/closeSingle/${id}`)
			.then((response) => {
				 console.log(response.data);
				 setDisabled(false);
...
async function generateNominalRoll() {
			setDisabled(true);
				...
				axios.get('/paySummarys/generateNominalRoll/')
					.then((response) => {
  ...
  const columns = useMemo(
    () => [
		 
       {
        ...
        Cell: ({ row }) => (
          <div>
             {row.original.action && row.original.action==='close' &&
              <button className=" w-16 m-0 p-0 " onClick={() => reclose(row.original.id)} disabled={row.original.disabled}>
             ...
             {row.original.action && row.original.action==='reclose' &&
              <button className=" w-16 m-0 p-0 " onClick={() => reclose(row.original.id)} disabled={row.original.disabled}>
              {row.original.disabled && (
						<span className="spinner-grow spinner-grow-sm"></span>
					)
            ...
            {' '}
            {row.original.action && (row.original.action==='reclose' || row.original.action==='mps') &&
              <button className=" w-16 m-0 p-0 " onClick={() => mps(row.original.id)} disabled={row.original.mpsDisabled}>
      ...
      }, 
      {
        ...
        Cell: ({ row }) => (
				...
				<div>
				{row.original.employee.cdaoNo}{row.original.employee.checkDigit}<br/>	 	 
  ...
  return (
      ...
      <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
				...
				<div className="-mt-2 max-h-1 py-0 ml-0">
					<TablePage columns={columns} data={data} newpage={page}
          ...
          <div>
            ...
            <div>
             <input
              ...
              onKeyPress={handleKeyPress} placeholder="mm/yyyy"
              className="pl-2 -ml-2 inputField flex-initial"
            /></div>
            <div>
             <input
              ...
              onKeyPress={handleKeyPress} placeholder="rank code"
              className="pl-2 -ml-2 inputField flex-initial"
            /></div>
            <div>
            <button
            ...
            <div/>
						<button className="w-46 ml-8 p-0 h-2 -mt-2" type="submit" onClick={generateNominalRoll} className="w-36 m-0 p-0">Generate Nominal Roll</button>
...
export default withRouter(PaySummaryLwList);
