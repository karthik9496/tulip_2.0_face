...
const AdjustmentEdit = () => {
	...
	useEffect(() => {
		...
		if (id !== 'new') {
			async function fetchData() {
				let record = '';
				await axios.get('/adjustments/' + id)
					.then((response) => {
						...
						'payCode', 'fromDate', 'toDate', 'amount', 'adjCb', 'monthEnding', 'pageNo', 'recordStatus',
						 ...
						 'approved',  'remarks', 'hoo', 'hooDate', 'paid','module' ,'action'		
					];
	...
	const onSubmit = (data, event) => {
		...
		if(disabled)
			...
			setDisabled(true);
		if (data.id) {
			axios.put("/adjustments/" + data.id, data)
				.then((response) => { 
...
export default withRouter(AdjustmentEdit);