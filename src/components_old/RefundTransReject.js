...
const RefundTransReject = () => {
	...
	useEffect(() => {
		...
		if (id !== 'new') {
			...
			async function fetchData() {
				let record = '';
				await axios.get('/refundTrans/' + id)
					.then((response) => {
						...
						const fields = [
							...
							'rejectionReason','uploadFileName'
	...
	useEffect(() => {
		...
		async function fetchUsrData() {
			...
			await axios.get(`/refundTrans/userDesg`)
				.then((response) => {
	...
	const onSubmit = (data, event) => {
		...
		if (data.id) {
			axios
				.put("/refundTrans/reject/" + data.id, data)
				.then((response) => {
	...
	const rollBack=(e)=>{
		console.log("-------------rollback---------:" + id);
			axios.put(`/refundTrans/rollBack/${id}`)
				.then((response) => {
					setMesg(response.data);
					setDisabled(false);
				})
...
export default withRouter(RefundTransReject);