...
function QstRejectedList() {
	...
	useEffect(() => {
		...
		async function fetchData() {
			...
			await axios.get('/qstTrans/rejectedList/qst?search=' + search)
	...
	const columns = useMemo(() => [
		{
			...
			Cell: ({ row }) => (
				...
				{(usrLevel<30 && (row.original.recordStatus==='D' || row.original.recordStatus==='I'))&&
					<Link to={"/qstTrans/" + row.original.id}>
		...
		},
		{
			...
			Cell: ({ row }) => (
				<div>
					...
					<div>
						<label>Rent Amount Total : {row.original.rentAmountTotal}</label>
					</div>
				</div>
			),
		},
		{
			...
			Cell: ({ row }) => (
				<div>
					...
					<div>
						<label>Fur Amount Total : {row.original.furAmountTotal}</label>
					</div>
				</div>
			),
		},
		...
		},
		{
            ...
            Cell: ({ row }) => (
				<div>
					...
					{data[row.index]['manuallyChecked'] === true ? 'Y' : 'N'}
				</div>
			)
		},
		...
		<div className="min-h-screen bg-gray-100 text-gray-900">
			<main className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
				<div className="mt-2 ml-4">
					<h1 className="text-xl font-semibold">Quarter Statistics Transactions-Rejected(IOR)</h1>
...
export default withRouter(QstRejectedList);
